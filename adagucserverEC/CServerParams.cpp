...
bool CServerParams::isAutoResourceCacheEnabled(){
  if(autoResourceCacheEnabled==-1){
    ...
    if(cfg->AutoResource.size()>0){
      if(cfg->AutoResource[0]->attr.enablecache.equals("all"))autoResourceCacheEnabled = 1;
      if(cfg->AutoResource[0]->attr.enablecache.equals("header"))autoResourceCacheEnabled = 1;
    }
  }
